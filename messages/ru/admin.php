<?php
return [
    'Friday' => 'Пятница',
    'Saturday' => 'Суббота',
    'Sunday' => 'Воскресенье',
    'Monday' => 'Понедельник',
    'Tuesday' => 'Вторник',
    'Wednesday' => 'Среда',
    'Thursday' => 'Четверг',
    'INCOME' => 'Доход',
    'CIRCULATION' => 'Оборот',
    'INCOME_FOR' => 'Доход за {month} {year}г.',
    'CIRCULATION_FOR' => 'Оборот за {month} {year}г.',
    'MSG_ORDER_DELETED' => 'Заказ помечен как удален. Этот заказ виден только Администратор сайита.',
    'ORDER_ISDELETED' => 'Заказ удален.',
    'STATUS_NEW_NOTIFY' => 'Данный статус присваивается новому заказу',
    'ORDER_TITLE_UPDATE' => 'Редактирование заказа №{id}',
    'ORDER_TITLE_CREATE' => 'Создание заказа',
    'MSG_BUYONECLICK' => 'Заказ оформлен через кнопку купить в 1 клик.',
    'CREATE_ORDER' => 'Добавить заказ',
    'ORDERS' => 'Заказы',
    'ORDERS_LIST' => 'Список заказов',
    'DELIVERY' => 'Доставка',
    'PAYMENTS' => 'Оплата',
    'ORDER_PRODUCTS' => 'Товары заказа',
    'STATS' => 'Статистика',
    'STATUSES' => 'Статусы заказов',
    'CREATE_STATUSES' => 'Создание статуса',
    'UPDATE_STATUSES' => 'Редактирование статуса',
    'PROMOCODE' => 'Промо-код',
    'OVER_PERIOD' => 'за период',
    'ORDER_PRINT' => 'Печать заказа',
    'ORDER_VIEW' => 'Просмотр заказа',
    'CREATE_PROMOCODE' => 'Добавить промо-код',
    'ERR_DELETE_STATUS' => 'Ошибка удаления статуса. Он используется заказами.',
    'ERR_ORDER_PRODUCT_EXISTS' => 'Ошибка, товар в этом заказе уже существует.',
    'NO_STATUSES' => 'Статус не найден.',
    'ALERT_CREATE_PRODUCT' => 'Товары можно будет добавить после создание заказа.',
    'ORDER_HISTORY' => 'История изменения заказа',
    'HISTORY' => 'История заказов',
    'HISTORY_EMPTY' => 'Список истории пустой.',
    'BEFORE' => 'До',
    'AFTER' => 'После',
    'SUCCESS_ADD_PRODUCT_ORDER' => 'Товар успешно добавлен в заказ.',
    'QUANTITY' => 'Количество',
    'ORDERED_PRODUCTS' => 'Заказанные продукты',
    'HISTORY_RESET' => 'Сбросить фильтры',
    'SAVE_PDF' => 'Сохранить в PDF',
    'HISTORY_DELETE_PRODUCT' => 'Удалил товар',
    'HISTORY_CHANGE_PRODUCT' => 'Изменил товар',
    'HISTORY_CREATE_PRODUCT' => 'Добавил товар',
    'ERR_DEL_DELIVERY' => 'Ошибка удаления способа доставки. Он используется заказами.',
    'NO_FOUND_DELIVERY' => 'Способ доставки не найден.',
    'EDIT_USER' => 'Редактировать пользователя',
    'ORDER_NOT_FOUND' => 'Заказ не найден.',
    'STATUS_CHANGE_NOTIFY' => 'После изменение статуса клиент будет уведомлен об изменение статуса.',
    'PRINT_PDF' => 'Печать (pdf)',
    'QUANTITY_CHANGED' => 'Количество <strong>{0}</strong> успешно изменено.',
    'LINK_ORDER_FOUND_USER' => 'Связать заказ с найденным пользователем?',
    'TO_TIE' => 'Связать',
    'SIMILARTY_IN' => 'Сходство по',
    'MATCH_USER' => 'Найдено совпадение &mdash; связать с этим заказом?',
    'CALL_CONFIRM' => 'Мне можно не звонить!',
    'ORDER_UNREGISTERED_USER' => 'Заказ оформлен <strong>незарегистрированным</strong> пользователем',
    'CALL' => 'Позвонить'
];
