<?php
return [
    'COST_DELIVERY' => 'Shipping cost',
    'MY_ORDERS' => 'My Orders',
    'MODULE_NAME' => 'Cart',
    'CART_EMPTY' => 'Cart is empty',
    'CART_EMPTY_HINT' => 'Your cart is empty',
    'CART_EMPTY_BTN' => 'Return to shopping',
    'WP_COUNT' => 'New orders ({num})',
    'SUCCESS_ORDER' => 'Thank you. Your order is accepted.',
    'SUCCESS_ADDCART' => 'The product <strong> {product_name} </strong> was successfully added to the cart.',
    'ERROR_PRODUCT_NO_FIND' => 'Error. Product not found. ',
    'BUY' => 'Buy',
    'ERROR_REGISTER' => 'Registration error',
    'BUY_SET' => 'Buy Bundle',
    'DETAIL_ORDER_VIEW' => 'You can view the order details on the page',
    'PAYMENT_ORDER' => 'Payment for order No. {id}',
    'VIEW_ORDER' => 'View order number {id}',
    'ERROR_ORDER_NO_FIND' => 'Error. Order not found. ',
    'TOTAL_PAY' => 'Total Payable',
    'BUTTON_CHECKOUT' => 'Checkout',
    'USER_DATA' => 'Personal data',
    'TABLE_SUM' => 'Total ({currency})',
    'TABLE_PRODUCT' => 'Product',
    'PAYMENT' => 'Payment',
    'COST' => 'Cost',
    'ERROR_SELECT_VARIANT' => 'An option must be selected',
    'ORDER_PRICE' => 'Order amount',
    'DELIVERY_PRICE' => 'Delivery price',
    'DELIVERY_PAYMENT' => 'Delivery and Payment',
    'SUCCESS_PRODUCT_CART_DELETE' => 'The item was successfully removed from the cart',
    'ORDER_STATUS' => 'Order status',
    'DELIVERY' => 'Delivery',
    'MAIL_TABLE_TH_PRODUCT' => 'Product',
    'QUANTITY' => 'Quantity',
    'PROMO_CODE_ACTIVATED' => 'Promo code activated',
    'PRICE_PER_UNIT' => 'Price per unit',
    'TOTAL_PRICE' => 'Total price',
    'SELECT_SYSTEM_PAYMENT' => '&mdash; Payment system &mdash;',
    'TOTAL_COST' => 'Total cost',
    'MAIL_ADMIN_SUBJECT' => 'A new order was issued #{0}',
    'MAIL_CHANGE_STATUS_SUBJECT' => 'The status of your order No. {0} has been changed',
    'MAIL_CLIENT_SUBJECT' => 'You have placed order #{0}',
    'MAIL_TTN_SUBJECT' => 'Your TTN order number {0}',
    'NOTIFICATION_USER_TEXT' => 'User <b>{username}</b> placed an order.<br/> <b>{num, plural, =0{empty} =1{One product} one{# product} few{# goods} many{# goods} other{# product}}</b> for the amount <b>{total} {currency}</b>',
    'NOTIFICATION_GUEST_TEXT' => 'Visitor <b>{username}</b> placed an order.<br/> <b>{num, plural, =0{empty} =1{One product} one{# product} few{# goods} many{# goods} other{# product}}</b> for the amount <b>{total} {currency}</b>',
    'RECEIVE_BONUSES' => 'You will receive bonuses',
    'CANCEL_BONUSES' => 'Cancel bonuses'
];
